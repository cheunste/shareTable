...
(function() {
  ...
  window.sharejs.extendDoc('attach_radio', function(elem) {
    ...
    //  }
    //};
	this.on('change',insert_listener=function(){
    ...
    genOp = function(event) {
      ...
      return onNextTick(function() {
        if (elem.value !== prevvalue) {
          ...
          return applyChange(doc, doc.value, elem.value);
        }
      });
    };
    ...
    //  } else {
    //    elem.attachEvent('onclick' + event, genOp);
    //  }
    //}
	_ref = ['onclick','onchange'];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      event = _ref[_i];
      if (elem.addEventListener) {
        elem.addEventListener(event, genOp, false);
      } else {
        elem.attachEvent(event, genOp);
      }
    }
    ...
    //    } else {
    //      _results.push(elem.detachEvent('onclick' + event, genOp));
    //    }
    //  }
    //  return _results;
    //};
